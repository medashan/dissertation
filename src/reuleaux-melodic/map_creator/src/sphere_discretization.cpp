...
//This method creates only quater area of the sphere
octomap::OcTree* SphereDiscretization::generateBoxTree2(const octomap::point3d& origin, float diameter, float resolution)
{
  octomap::OcTree* tree = new octomap::OcTree(resolution / 2);
  octomap::Pointcloud p;
  for (float x = origin.x(); x <= origin.x() + diameter * 1.5; x += resolution)
  {
    for (float y = origin.y()- diameter * 1.5; y <= origin.y() + diameter * 1.5; y += resolution)
    {
