...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    octomap::OcTree *tree = sd.generateBoxTree2(origin, r, resolution);
    ...
    ofstream myfile;
    myfile.open("reachable_points.txt");
         ...
         ++it)  // for all the spheres in workspace
    {
      ...
      if (it->second <= 20)  // All the spheres that have reachability less or equal to 20
      {
        ...
        myfile << i <<"-"<< sphere_center << '\n' << optimal_pose_pca.orientation<<'\n' << "Reachability value -" << it->second << '\n'; 
